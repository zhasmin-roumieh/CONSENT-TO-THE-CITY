// Maps each location ID to its Wikipedia article title.
// Used to fetch the official thumbnail from Wikipedia's API.
export const WIKI_TITLES = {
  // ── Berlin ──────────────────────────────────────────
  brandenburger:       'Brandenburg Gate',
  tiergarten:          'Tiergarten (park)',
  hackescher:          'Hackescher Markt',
  tempelhof:           'Tempelhofer Feld',
  kreuzberg:           'Kreuzberg',
  museumsinsel:        'Museum Island',
  'east-side-gallery': 'East Side Gallery',
  'potsdamer-platz':   'Potsdamer Platz',
  alexanderplatz:      'Alexanderplatz',
  'checkpoint-charlie':'Checkpoint Charlie',
  gendarmenmarkt:      'Gendarmenmarkt',
  oberbaumbruecke:     'Oberbaum Bridge',
  'prenzlauer-berg':   'Prenzlauer Berg',
  mauerpark:           'Mauerpark',
  charlottenburg:      'Charlottenburg Palace',
  neukoelln:           'Neukölln',
  reichstag:           'Reichstag building',
  teufelsberg:         'Teufelsberg',
  treptower:           'Soviet War Memorial (Treptower Park)',
  gorlitzer:           'Görlitzer Park',
  'karl-marx-allee':   'Karl-Marx-Allee',

  // ── Paris ───────────────────────────────────────────
  eiffel:              'Eiffel Tower',
  marais:              'Place des Vosges',
  bastille:            'Place de la Bastille',
  canal:               'Canal Saint-Martin',
  montmartre:          'Montmartre',
  'les-halles':        'Les Halles',
  'pere-lachaise':     'Père Lachaise Cemetery',
  pigalle:             'Pigalle, Paris',

  // ── Tokyo ────────────────────────────────────────────
  shinjuku:            'Shinjuku',
  harajuku:            'Harajuku',
  shibuya:             'Shibuya Crossing',
  ueno:                'Ueno Park',
  tsukiji:             'Tsukiji',
  yanaka:              'Yanaka, Tokyo',
  asakusa:             'Asakusa',
  akihabara:           'Akihabara',

  // ── London ──────────────────────────────────────────
  borough:             'Borough Market',
  southbank:           'Tate Modern',
  'hyde-park':         'Hyde Park, London',
  'brick-lane':        'Brick Lane',
  trafalgar:           'Trafalgar Square',
  brixton:             'Brixton',
  'canary-wharf':      'Canary Wharf',

  // ── New York ─────────────────────────────────────────
  centralpark:         'Central Park',
  highline:            'High Line',
  'times-square':      'Times Square',
  'brooklyn-bridge':   'Brooklyn Bridge',
  harlem:              'Harlem',
  'washington-square': 'Washington Square Park',
  'coney-island':      'Coney Island',

  // ── Cairo ────────────────────────────────────────────
  tahrir:              'Tahrir Square',
  'khan-khalili':      'Khan el-Khalili',
  'nile-corniche':     'Nile Corniche',
  'city-of-dead':      'Mosque of Ibn Tulun',
  giza:                'Giza pyramid complex',
  'al-azhar-park':     'Al-Azhar Park',

  // ── Mumbai ───────────────────────────────────────────
  dharavi:             'Dharavi',
  'marine-drive':      'Marine Drive, Mumbai',
  gateway:             'Gateway of India',
  'dhobi-ghat':        'Dhobi Ghat (Mumbai)',
  juhu:                'Juhu',
  colaba:              'Sassoon Docks',

  // ── Beirut ───────────────────────────────────────────
  'martyrs-square':    'Mohammad Al-Amin Mosque',
  hamra:               'Hamra, Beirut',
  gemmayzeh:           'Achrafieh',
  'corniche-beirut':   'Corniche Beirut',
  'mar-mikhael':       'Sursock Museum',
  raouche:             'Raouché',
  'bourj-hammoud':     'Bourj Hammoud',
  achrafieh:           'Achrafieh',
  'port-beirut':       'Port of Beirut',
  badaro:              'Badaro, Beirut',
  shatila:             'Shatila refugee camp',
  'national-museum':   'National Museum of Beirut',
};
